...
BOOL CALLBACK MainDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:		//ダイアログ初期化
		...
		hPict3 = GetDlgItem(hDlg, IDC_STATIC3);
		WinInitialize(NULL, hDlg, (HMENU)110, "TEST3", hPict3, WndProc, &hwnd3); //初期化
		hPict4 = GetDlgItem(hDlg, IDC_STATIC4);
		WinInitialize(NULL, hDlg, (HMENU)110, "TEST4", hPict4, WndProc, &hwnd4); //初期化
...
UINT WINAPI TFunc(LPVOID thParam)
{
	...
	while (Flag == TRUE) {
		...
		//データの読み込み
		if (fscanf(fp, "%lf   %lf   %lf   %lf   %lf", &data0,&data1,&data2,&data3,&data4) == EOF) {
		...
		if (x >= rect.right - 50) {
			...
			Rectangle(hdc3, -1, -1, rect.right + 2, rect.bottom + 2);	//四角
			MoveToEx(hdc3, 40, rect.bottom / 2, NULL);			//カレントポジション
			LineTo(hdc3, rect.right - 50, rect.bottom / 2);	//X軸
			MoveToEx(hdc3, 40, rect.top + 10, NULL);			//カレントポジション
			LineTo(hdc3, 40, rect.bottom - 10);				//Y軸
			//横軸のタイトル
			SetTextColor(hdc3, RGB(255, 255, 255));
			SetBkColor(hdc3, RGB(0, 0, 0));
			TextOut(hdc3, rect.right * 0.42, rect.bottom * 0.8, TEXT("Time(s)"), 7);
			Rectangle(hdc4, -1, -1, rect.right + 2, rect.bottom + 2);	//四角
			MoveToEx(hdc4, 40, rect.bottom / 2, NULL);			//カレントポジション
			LineTo(hdc4, rect.right - 50, rect.bottom / 2);	//X軸
			MoveToEx(hdc4, 40, rect.top + 10, NULL);			//カレントポジション
			LineTo(hdc4, 40, rect.bottom - 10);				//Y軸
			//横軸のタイトル
			SetTextColor(hdc4, RGB(255, 255, 255));
			SetBkColor(hdc4, RGB(0, 0, 0));
			TextOut(hdc4, rect.right * 0.42, rect.bottom * 0.8, TEXT("Time(s)"), 7);
			x = rect.left + 40;
			MoveToEx(hdc1, x, y1, NULL);
			MoveToEx(hdc2, x, y2, NULL);
			MoveToEx(hdc3, x, y3, NULL);
			MoveToEx(hdc4, x, y4, NULL);

			SelectObject(hdc1, hOldPen);
			SelectObject(hdc2, hOldPen);
			SelectObject(hdc3, hOldPen);
			SelectObject(hdc4, hOldPen);
			DeleteObject(hPen);
			hPen = CreatePen(PS_SOLID, 2, colorR);		//ペン生成
			hOldPen = (HPEN)SelectObject(hdc1, hPen);		//ペン設定
			hOldPen = (HPEN)SelectObject(hdc2, hPen);		//ペン設定
